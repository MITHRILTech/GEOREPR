function [SH, SHAC, SAC, SCa, SCaHCO3, SH2CO3, SHCO3, SCO3] = SCalciteAC(T, unit,...
    cNaCl0, cH0, cHAC0, cAC0, cCa0, cCaHCO30, cCaT0, cH2CO30, cHCO30, cCO30, cCT0, m, a0, sCaCO3, dt0)
...
cNaCl0(cNaCl0<0) = 0; cH0(cH0<0) = 0; 
cHAC0(cHAC0<0) = 0; cAC0(cAC0<0) = 0; cCa0(cCa0<0) = 0;
...
for i = 1:length(t)
    ...
    if i == 1
        ...
        %         if cH2SO4(:,:,i) == 0
%         [cHAC(:,:,i+1), cAC(:,:,i+1)] = eq1H2SO4(T, unit, cH(:,:,i), cACT(:,:,i), I(:,:,i));
        
        [cHAC(:,:,i+1), cH(:,:,i+1), cAC(:,:,i+1)] = eqHAC1(T, unit, cHAC(:,:,i), cH(:,:,i), cAC(:,:,i), I(:,:,i));
        ...
        aSO4(:,:,i) = caseI1 .* (cAC(:,:,i) .* 10 .^ (-0.509 * 1^2 * I(:,:,i).^0.5 ./ (1 + 3.28 * 0.4 * I(:,:,i).^0.5))) +...
            caseI2 .* (cAC(:,:,i) .* 10 .^ (-0.509 * (I(:,:,i).^0.5 ./ (1 + I(:,:,i).^0.5) - 0.3 * I(:,:,i))));
        aHSO4(:,:,i) = caseI1 .* (cAC(:,:,i) .* 10 .^ (-0.509 * (I(:,:,i).^0.5 ./ (1 + I(:,:,i).^0.5) - 0.3 * I(:,:,i)))) + ...
            caseI2 .* (cHAC(:,:,i) .* 10 .^ (-0.509 * (I(:,:,i).^0.5 ./ (1 + I(:,:,i).^0.5) - 0.3 * I(:,:,i))));
...
%     KdHSO4prime = K2H2SO4prime(Tab, unit, I(:,:,i));
    KdHACprime = 10 .^ (-6.634 + 0.01305*Tab + 1.236*I(:,:,i) - 2.272*10^-5*Tab.^2 -...
        0.00388*Tab.*I(:,:,i) + 2.866*I(:,:,i).^2+8.066*10^-12*Tab.^3 + 2.376*10^-...
        5*Tab.^2.*I(:,:,i)-0.0205*Tab.*I(:,:,i).^2);
...
%     [cHAC(:,:,i+1), cAC(:,:,i+1)] = eq1H2SO4(T, unit, cH(:,:,i), cACT(:,:,i), I(:,:,i)); %?
    ...
    cAC(:,:,i+1) = caseH1 .* (cHAC(:,:,i) + cAC(:,:,i))+...
        caseH1a .* caseH2 .* (cHAC(:,:,i) - (m1max(:,:,i)./V - aH(:,:,i)) + cAC(:,:,i))+...
        caseH3 .* (cAC(:,:,i+1));  %??
    cHAC(:,:,i+1) = 0*caseH1+...
        caseH1a .* caseH2 .* (cACT(:,:,i) - cAC(:,:,i))+...
        caseH3 .* (cHAC(:,:,i+1));
    
    aSO4(:,:,i+1) = cAC(:,:,i+1) .* gammaSO4(:,:,i);
    aHSO4(:,:,i+1) = cHAC(:,:,i+1) .* gammaHSO4(:,:,i);
    
%     cH(:,:,i+1) = caseH1 .* (aH(:,:,i+1) ./ gammaH(:,:,i))+...
%         caseH1a .* caseH2 .* (KdHSO4prime .* cHSO4(:,:,i+1) ./ cSO4(:,:,i+1))+...
%         caseH3 .* (cH(:,:,i));
    cHa = caseH1 .* (aH(:,:,i+1) ./ gammaH(:,:,i));
    cHb = caseH1a .* caseH2 .* (KdHACprime .* cHAC(:,:,i+1) ./ cAC(:,:,i+1));
    cHb(isnan(cHb)) = 0; 
    cHc = caseH3 .* (cH(:,:,i));
...
%             0 * casePost0;
    ...
    cACT(:,:,i+1) =  cHAC(:,:,i+1) + cAC(:,:,i+1);
    
    
%     cHdebug = cH(:,:,i+1); cOHdebug = cOH(:,:,i+1); cSO4debug = cAC(:,:,i+1);
%     cCadebug = cCa(:,:,i+1); cCaHCO3debug = cCaHCO3(:,:,i+1); cHCO3debug = cHCO3(:,:,i+1);
%     cCO3debug = cCO3(:,:,i+1); 
    
    Idebug = 0.5*(2*cNaCl0 + cHAC(:,:,i+1) + cH(:,:,i+1) + cOH(:,:,i+1) + 4*cAC(:,:,i+1) + 4*cCa(:,:,i+1) +...
